...
import AlgoritmRulare.Paralel.DelataSteppingSingle.communResorc.BBaketInfo;
...
import AlgoritmRulare.Paralel.DelataSteppingSingle.communResorc.PunctValuClass;
import java.io.File;
   ...
   */
    ...
    private LinkedList<PunctValuClass> findRequests(BBaketInfo galetaCurenta, int tipDeMuchii) {//-1 light si 1 hevy
        ...
        while(baketInterator.hasNext()){
            ...
            comRes.afis(currentBaket.getLinie(), currentBaket.getColoana());
    ...
    private void afis(Punct punct) {
        ...
        if(punctInfo!=null){
            ...
            comRes.afis(punctInfo.getLinie(), punctInfo.getColoana());
        }
    ...
    private void crateThreads() {
     //   System.out.println(nrOfThreads);
        ...
        for(int i=1; i<this.nrOfThreads; i++){
            DeltaThread td = new DeltaThread(this.barrierBefore,this.barrierAfter,i,comRes);
            new Thread(td).start();
        }
    }
