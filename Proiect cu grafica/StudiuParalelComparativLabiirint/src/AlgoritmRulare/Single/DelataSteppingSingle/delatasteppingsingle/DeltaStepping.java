...
import AlgoritmRulare.Single.DelataSteppingSingle.communResorc.BBaketInfo;
import AlgoritmRulare.Single.DelataSteppingSingle.communResorc.CommunResurces;
import AlgoritmRulare.Single.DelataSteppingSingle.communResorc.Punct;
import AlgoritmRulare.Single.DelataSteppingSingle.communResorc.PunctValuClass;
import java.io.File;
...
class DeltaStepping {
    ...
    void initilize(TestData td,Graphics g) {
   ...
   */
    ...
    private LinkedList<PunctValuClass> findRequests(BBaketInfo galetaCurenta, int tipDeMuchii) {//-1 light si 1 hevy
        ...
        while(baketInterator.hasNext()){
            ...
            */
             ...
             comRes.delay(comRes.getSecunda());
                comRes.getG().setColor(Color.GREEN);
                comRes.getG().fillRect(currentBaket.getColoana()*comRes.getfPrecis()+1 ,currentBaket.getLinie()*comRes.getfPrecis()+1 , comRes.getfPrecis()-1 , comRes.getfPrecis()-1);
    ...
    private void afis(Punct punct) {
        ...
        comRes.delay(comRes.getSecunda());
        comRes.getG().setColor(Color.RED);
        comRes.getG().fillRect(punct.getColoana()*comRes.getfPrecis()+1 , punct.getLinie()*comRes.getfPrecis()+1 , comRes.getfPrecis()-1 , comRes.getfPrecis()-1);
