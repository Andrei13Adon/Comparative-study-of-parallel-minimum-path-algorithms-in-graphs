 ...
 */
public class LabirintCreaterAlgsKruskal {
    ...
    public void start() {
        try {
            ...
            GenerateNewMazeUsingKruskal usingKruskal = new GenerateNewMazeUsingKruskal(marimeLabirint);
       //     usingPrime.setMarimeLab(marimeLabirint);
            usingKruskal.generateNewMazeUsingKruskal(punctStart, punctStop);
            
            for(int i=0;i<marimeLabirint; i++){
                for(int j=0;j<marimeLabirint; j++)
                    if(usingKruskal.getLabirint()[i][j])
                        pw.print(1 + " ");
                    else 
                        pw.print(0 + " ");
                pw.println();
